...
func NewPageContent() NewEdit {
	return NewEdit{
		Action:       NewAction,
		Device:       storage.Device{},
		IPVersion:    4,
		IPPattern:    IPv4Pattern,
		ErrorMessage: "",
	}
}

func NewPageContentWithError(device storage.Device, errMsg string) NewEdit {
	newEdit := EditPageContent(device, errMsg)
	newEdit.Action = NewAction

	return newEdit
}

func EditPageContent(device storage.Device, errMsg string) NewEdit {
